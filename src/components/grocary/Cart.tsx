...
const Cart = () => {
  ...
  const getAddress = async () => {
    try {
      const { data }: AxiosResponse<GetAddressResponse> = await axios.get(
        "/api/address/get"
      );
      if (data.success) {
  ...
  }, [user, axios]); // Added axios to dependency array
  ...
  const placeOrder = async () => {
    try {
      ...
      if (paymentOption === "COD") {
        const { data }: AxiosResponse<PlaceOrderResponse> = await axios.post(
          "/api/order/cod",
          {
  ...
  return products.length > 0 && totalCartAmount() > 0 ? (
    <div className="flex flex-col md:flex-row py-16 max-w-6xl w-full px-6 mx-auto">
      <div className="flex-1 max-w-4xl">
        ...
        {cartArray.map((product) => (
          <div
            key={product._id} // Use a unique ID for the key
              ...
              <div>
                ...
                <div className="font-normal text-gray-500/70">
                  ...
                  <div className="flex items-center">
                    ...
                    >
                      {Array(
                        (cartItems[product._id] > 9 ? cartItems[product._id] : 9) + 1
                      ).fill(0).map((_, index) => (
      ...
      <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
        ...
        <div className="mb-6">
          ...
          <select
            onChange={(e: ChangeEvent<HTMLSelectElement>) => setPaymentOption(e.target.value)}
